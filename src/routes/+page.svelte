<script lang="ts">
	...
	// Gallery
	let videos = $state<Record<string, string | number>[]>([]);
	let galleryLoading = $state(true);
	let galleryError = $state('');

	// ── On mount: load gallery ─────────────────────────────────────────────────
	$effect(() => {
		fetchGallery();
	});
	...
	async function fetchGallery() {
		...
		galleryError = '';
		try {
			const res = await fetch('/api/videos?limit=24');
			const data = await res.json();
			if (data.videos) videos = data.videos;
		} catch {
			galleryError = 'Could not load gallery.';
		} finally {
	...
	async function copy(text: string, type: 'gif' | 'video') {
			...
			const cloudinaryResult = await new Promise<Record<string, unknown>>((resolve, reject) => {
				...
				xhr.upload.addEventListener('progress', (e) => {
					if (e.lengthComputable) {
						// Cloudinary upload = first 85% of total progress
	...
	<main class="max-w-5xl mx-auto px-4 py-10 space-y-12">
			...
			{#if !uploadedVideo && !uploading}
				...
				>
					...
					<div class="flex flex-col items-center justify-center gap-4 py-20 px-6 text-center select-none">
						...
						<div>
							<p class="text-lg font-semibold">Drag & drop your video here</p>
			...
			{#if uploadedVideo}
				<Card.Root class="border-2 border-primary/30 bg-primary/5 shadow-lg">
					...
					<Card.Content class="space-y-4">
						...
						<!-- Video link -->
						<div class="space-y-2">
							<label class="text-xs font-semibold uppercase tracking-wide text-muted-foreground flex items-center gap-1.5">
								<Film class="size-3.5" /> Video Link
							</label>
							<div class="flex gap-2">
								<div class="flex-1 bg-background border border-border rounded-md px-3 py-2 text-xs font-mono text-muted-foreground truncate">
									{uploadedVideo.originalUrl}
								</div>
								<Button
									variant="secondary"
									size="sm"
									onclick={() => copy(uploadedVideo!.originalUrl as string, 'video')}
								>
									{#if copiedVideo}
										<Check class="size-4 mr-1.5" /> Copied!
									{:else}
						...
						<div class="flex flex-wrap gap-2 pt-1">
							<Button
								href="/v/{uploadedVideo.shortId}"
								variant="outline"
								size="sm"
							>
								<ExternalLink class="size-4 mr-1.5" /> Open video page
							</Button>
							<Button
								variant="ghost"
								size="sm"
		...
		<section class="space-y-4">
			<div class="flex items-center justify-between">
				...
				{#if !galleryLoading}
					<Button variant="ghost" size="sm" onclick={fetchGallery} class="text-muted-foreground text-xs">
						Refresh
					</Button>
				{/if}
			</div>
			...
			{:else if galleryError}
				<p class="text-destructive text-sm">{galleryError}</p>
			...
			{:else}
				<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
					{#each videos as video}
						{@const v = video as Record<string, string | number>}
						<a
							href="/v/{v.shortId}"
							class="group relative rounded-xl overflow-hidden bg-muted border border-border
								hover:border-primary/50 hover:shadow-md transition-all duration-200 block"
						>
							<!-- Thumbnail -->
							...
							{#if v.duration}
								<div class="absolute bottom-10 right-2">
									<span class="text-[10px] bg-black/70 text-white rounded px-1.5 py-0.5 backdrop-blur-sm flex items-center gap-1">
										...
										{formatDuration(v.duration as number)}
									</span>
								</div>
							{/if}
							...
							<div class="p-2.5 bg-card">
								<p class="text-xs font-medium truncate">{v.title || 'Untitled'}</p>
								<div class="flex items-center justify-between mt-0.5 text-[10px] text-muted-foreground">
									<span class="flex items-center gap-0.5">
										<Eye class="size-2.5" />
										{(v.views as number).toLocaleString()}
									</span>
									<span>{formatDate(v.uploadedAt as string)}</span>
								</div>
							</div>
						</a>
					{/each}
